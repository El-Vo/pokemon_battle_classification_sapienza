...
class MostSuccessfulTeamsRadarVisualizer:
    ...
    def compute_team_win_counts(self) -> Counter[TeamKey]:
        ...
        for battle in self.data_source.data:
            ...
            team_key: TeamKey = tuple(
                sorted(pokemon["name"] for pokemon in team_details)
            )
    ...
    def plot(
        ...
        if not nodes:
            raise ValueError(
                "No Pokémon nodes available after filtering winning teams."
            )
        ...
        if edge_weight:
            ...
            for (p1, p2), weight in sorted(
                edge_weight.items(), key=lambda item: (-item[1], item[0])
            ):
                x1, y1 = positions[p1]
                x2, y2 = positions[p2]
                ...
                ax.plot(
                    [x1, x2], [y1, y2], color=color, linewidth=line_width, alpha=0.75
                )
