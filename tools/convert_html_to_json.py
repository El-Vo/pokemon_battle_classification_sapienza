...
class BattleHTMLConverter:
    ...
    def _initialize_move_data(self) -> Dict[str, Dict[str, Any]]:
        ...
        # Based on moves seen in sample_battle_train.json
        return {
            'icebeam': {'type': 'ICE', 'category': 'SPECIAL', 'base_power': 95, 'accuracy': 1.0, 'priority': 0},
            'sleeppowder': {'type': 'GRASS', 'category': 'STATUS', 'base_power': 0, 'accuracy': 0.75, 'priority': 0},
            'blizzard': {'type': 'ICE', 'category': 'SPECIAL', 'base_power': 120, 'accuracy': 0.9, 'priority': 0},
            'thunderbolt': {'type': 'ELECTRIC', 'category': 'SPECIAL', 'base_power': 95, 'accuracy': 1.0, 'priority': 0},
            'thunderwave': {'type': 'ELECTRIC', 'category': 'STATUS', 'base_power': 0, 'accuracy': 1.0, 'priority': 0},
            'recover': {'type': 'NORMAL', 'category': 'STATUS', 'base_power': 0, 'accuracy': 1.0, 'priority': 0},
            'bodyslam': {'type': 'NORMAL', 'category': 'PHYSICAL', 'base_power': 85, 'accuracy': 1.0, 'priority': 0},
            'reflect': {'type': 'PSYCHIC', 'category': 'STATUS', 'base_power': 0, 'accuracy': 1.0, 'priority': 0},
            'rest': {'type': 'PSYCHIC', 'category': 'STATUS', 'base_power': 0, 'accuracy': 1.0, 'priority': 0},
            'psychic': {'type': 'PSYCHIC', 'category': 'SPECIAL', 'base_power': 90, 'accuracy': 1.0, 'priority': 0},
            'explosion': {'type': 'NORMAL', 'category': 'PHYSICAL', 'base_power': 170, 'accuracy': 1.0, 'priority': 0},
            'earthquake': {'type': 'GROUND', 'category': 'PHYSICAL', 'base_power': 100, 'accuracy': 1.0, 'priority': 0},
            'hyperbeam': {'type': 'NORMAL', 'category': 'PHYSICAL', 'base_power': 150, 'accuracy': 0.9, 'priority': 0},
            'lovelykiss': {'type': 'NORMAL', 'category': 'STATUS', 'base_power': 0, 'accuracy': 0.75, 'priority': 0},
            'seismictoss': {'type': 'FIGHTING', 'category': 'PHYSICAL', 'base_power': 1, 'accuracy': 1.0, 'priority': 0},
            'doubleedge': {'type': 'NORMAL', 'category': 'PHYSICAL', 'base_power': 100, 'accuracy': 1.0, 'priority': 0},
            'counter': {'type': 'FIGHTING', 'category': 'PHYSICAL', 'base_power': 1, 'accuracy': 1.0, 'priority': -1},
            'selfdestruct': {'type': 'NORMAL', 'category': 'PHYSICAL', 'base_power': 130, 'accuracy': 1.0, 'priority': 0},
            'megadrain': {'type': 'GRASS', 'category': 'SPECIAL', 'base_power': 40, 'accuracy': 1.0, 'priority': 0},
            'substitute': {'type': 'NORMAL', 'category': 'STATUS', 'base_power': 0, 'accuracy': 1.0, 'priority': 0},
            'amnesia': {'type': 'PSYCHIC', 'category': 'STATUS', 'base_power': 0, 'accuracy': 1.0, 'priority': 0},
            'hypnosis': {'type': 'PSYCHIC', 'category': 'STATUS', 'base_power': 0, 'accuracy': 0.6, 'priority': 0},
            'softboiled': {'type': 'NORMAL', 'category': 'STATUS', 'base_power': 0, 'accuracy': 1.0, 'priority': 0},
            'fireblast': {'type': 'FIRE', 'category': 'SPECIAL', 'base_power': 120, 'accuracy': 0.85, 'priority': 0},
            'agility': {'type': 'PSYCHIC', 'category': 'STATUS', 'base_power': 0, 'accuracy': 1.0, 'priority': 0},
            'drillpeck': {'type': 'FLYING', 'category': 'PHYSICAL', 'base_power': 80, 'accuracy': 1.0, 'priority': 0},
            'sing': {'type': 'NORMAL', 'category': 'STATUS', 'base_power': 0, 'accuracy': 0.55, 'priority': 0},
            'headbutt': {'type': 'NORMAL', 'category': 'PHYSICAL', 'base_power': 70, 'accuracy': 1.0, 'priority': 0},
            'stunspore': {'type': 'GRASS', 'category': 'STATUS', 'base_power': 0, 'accuracy': 0.75, 'priority': 0},
            'razorleaf': {'type': 'GRASS', 'category': 'SPECIAL', 'base_power': 55, 'accuracy': 0.95, 'priority': 0},
        }
    
    def convert_html_to_json(self, html_path: str, player_perspective: str = 'p1', 
                            battle_id: int = 0) -> Dict[str, Any]:
        ...
        battle_log_script = soup.find('script', {'type': 'text/plain', 'class': 'battle-log-data'})
    ...
    def _parse_battle_log(self, log_lines: List[str], player_perspective: str, 
                         battle_id: int) -> Dict[str, Any]:
            ...
            if command == 'switch':
                ...
                if player.startswith('p1'):
                    ...
                    if not result['p2_lead_details']:
                        result['p2_lead_details'] = self._get_pokemon_details(pokemon_name)
            
            # Turn marker
            elif command == 'turn':
                # Save previous turn if it exists
                if current_turn > 0 and p1_active and p2_active:
                    result['battle_timeline'].append(self._create_turn_entry(
                        current_turn, p1_active, p1_hp, p1_status, p1_effects, p1_boosts,
                        turn_data.get('p1_move'), p2_active, p2_hp, p2_status, p2_effects,
                        p2_boosts, turn_data.get('p2_move')
                    ))
    ...
    def _create_turn_entry(self, turn: int, p1_pokemon: str, p1_hp: float,
                          p1_status: str, p1_effects: List[str], p1_boosts: Dict[str, int],
                          p1_move: Optional[str], p2_pokemon: str, p2_hp: float,
                          p2_status: str, p2_effects: List[str], p2_boosts: Dict[str, int],
                          p2_move: Optional[str]) -> Dict[str, Any]:
