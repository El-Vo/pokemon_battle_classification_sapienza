...
def create_training_features() -> tuple[DataFrame,list]:
    ...
    train_feature_names = [col for col in train_df.columns if col not in ['battle_id', 'player_won']]
    
    return train_df,train_feature_names
    
def train_logistic_regression_model(train_dataframe:DataFrame, train_feature_names:list, log_results:bool = False) -> SklearnLogisticRegression:
...
def test_logistic_regression_model(model: SklearnLogisticRegression, features: list) -> None:
...
def predict_single_battle(model: SklearnLogisticRegression,features: list, filename: str):
    test_data_importer = ImportSource()
    ...
    contributions = [(feature, coef * value, value) for feature, coef, value in feature_importance]
